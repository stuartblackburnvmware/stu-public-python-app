...
```
pack build stu-public-python-app --path . --builder ghcr.io/vmware-tanzu/function-buildpacks-for-knative/functions-builder:0.3.1 --env BP_FUNCTION=func.main
```

Where `stu-public-python-app` is the name of your runnable function image.

Then run via Docker:

```
docker run -it --rm -p 8080:8080 stu-public-python-app
```
...
```
tanzu apps workload create stu-public-python-app -f config/workload.yaml \
  --local-path . \
  --type web
```
...
```
tanzu apps workload get stu-public-python-app
```
